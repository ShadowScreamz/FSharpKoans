...
module ``08: The Good Kind of Discrimination`` = 
    ...
    let ``04 A discriminated union can refer to itself (i.e., it can be recursive).`` () =
        ...
        let a = Node("0", Node("1", Node("2", Empty, Empty), Node("3", Empty, Empty)), Node("4", Empty, Empty)) // <-- you may want to spread this over multiple lines and/or let-bindings ...!
        depth a |> should equal 3
