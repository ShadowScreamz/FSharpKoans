    ...
    *)
        ...
        let length (xs : 'a list) : int =
            let rec length list count =
                     ...
                     | _::tail -> 
                                 ...
                                 length tail count
            length xs 0 // write a function to find the length of a list
    ...
    let ``05 Fixed-function mapping, the hard way (part 1).`` () =
        let map (xs : int list) : int list =
            xs |> List.map (fun x -> x + 1) // write a function which adds 1 to each element
    ...
    let ``06 Fixed-function mapping, the hard way (part 2).`` () =
        let map (xs : int list) : int list =
             xs |> List.map (fun x -> x * 2) // write a function which doubles each element
   ...
   *)
    ...
    let ``09 Specified-function filtering, the hard way`` () =
        let filter (f : 'a -> bool) (xs : 'a list) : 'a list =
            xs |> List.filter f // write a function which filters based on the specified criteria
    ...
    let ``11 Fixed-function filtering, the hard way`` () =
        let filter (xs : int list) : int list =
            xs |> List.filter (fun x -> x % 2 <> 0) // write a function to filter for odd elements only.
...
*)
    ...
    let ``14 A fold which sums a list`` () =
        let fold initialState xs =
            List.fold (fun acc elem -> acc + elem) initialState xs // write a function to do what's described above
    ...
    let ``15 A fold which multiplies a list`` () =
        let fold initialState xs =
             List.fold (fun acc elem -> acc * elem) initialState xs // write a function to multiply the elements of a list
